...
// @desc    Get all tasks (Admin: all, User: only assign tasks)
// @route   GET /api/tasks/
// @access  Private
const getTasks = async (req, res) => {
    try {
        const {status} = req.query
        let filter = {} // If filter is empty then find all tasks for admin and for member only their assigned tasks
...
const getDashboardData = async (req, res) => {
    try {
        ...
        const taskDistribution = taskStatuses.reduce((acc, status) => {
            const formatKey = status.replace(/\s+/g, "") // remove spaces for response keys
...
export {getTasks, getTaskById, createTask, updateTask, deleteTask, updateTaskStatus, updateTaskChecklist, getDashboardData, getUserDashboardData}